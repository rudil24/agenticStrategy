...
* __RULES__: **(Google Antigravity specific)** Global rules of engagement, constraints, and coding standards that the agent automatically reads. *(Analogous to Claude's Project Knowledge or Custom Instructions).*
* __WORKFLOWS__: **(Google Antigravity specific)** Well-defined, step-by-step repeatable processes (like project kickoffs) that the agent can auto-run. *(Analogous to step-by-step task lists or standard operating procedures given to Claude).*
* __COMMANDS__: **(Claude specific)** Standardized CLI commands, slash commands, or custom tools defined for Claude to quickly automate actions. *(Analogous to Antigravity's native `run_command` scripting or custom Agent Skills).*
* __HOOKS__: **(Claude specific)** Automation triggers that run [before or after](claudeHooks.md) certain actions during the development lifecycle. *(Analogous to automated steps grouped directly into Antigravity Workflows).*
